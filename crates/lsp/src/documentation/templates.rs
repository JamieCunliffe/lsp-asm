...
lazy_static! {
    static ref TEMPLATE_CONFIG: ParserConfig = ParserConfig {
        ...
        registers: Some(&crate::documentation::registers::DOCUMENTATION_REGISTERS),
    };
}
...
pub(crate) fn find_correct_instruction_template<'a>(
    node: &SyntaxNode,
...
pub(crate) fn find_potential_instruction_templates<'a>(
    node: &SyntaxNode,
    instructions: &'a [Instruction],
    lookup: &Option<impl Registers>,
...
pub(crate) fn instruction_from_template<'a>(
    instructions: &'a [Instruction],
    ...
    match elements {
        (a, t) if a.kind() == SyntaxKind::REGISTER && t.kind() == SyntaxKind::REGISTER => {
            if let Some(lookup) = lookup {
                let actual = a.as_token().unwrap();
                let template = t.as_token().unwrap();
